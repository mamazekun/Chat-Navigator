...
- 支持按关键词过滤问题
- 点击问题后平滑滚动到对应消息
- 支持对历史问题一键“追问”：自动将选中历史问题与当前输入拼接为追问模板并写回输入框
- 根据当前滚动位置自动高亮正在浏览的问题
...
- 鼠标移出后，面板延迟收起
- 面板顶部提供搜索框
- 每条问题右侧提供“追问”按钮，可基于该问题快速生成后续提问草稿
- 问题较多时，列表区域自动启用滚动
...
- `src/content/dom-parser.ts`
  站点适配、问题提取、输入框读写逻辑，当前实现了 ChatGPT 和 Gemini 适配器
...
- 点击某条问题的“追问”按钮后，插件会读取当前输入框内容
- 如果目标问题是最新一条用户问题，则保持当前输入不变
- 如果目标问题是历史问题，则按以下模板写入输入框：
...
- 输入框回填依赖站点前端事件机制；若目标站点编辑器行为调整，可能需要更新选择器与事件派发逻辑
